...
On the future of Jupyter notebooks, _Mathematica_ also can serve as a great source of inspiration. In Jupyter, code blocks are in either Python and R, text is in Markdown and LaTeX, and figures are in image formats. In _Mathematica_, they are all expressed in the same _Mathematica_ symbolic representation. For a LISP-like language like _Mathematica_, there is no distinction between code and data, so it's natural to mix them together. A lot of cool things can be done this way. I hope something like it makes its way to Jupyter.
