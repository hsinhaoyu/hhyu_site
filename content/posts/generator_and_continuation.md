...
But very briefly, `yield` in Python is often presented as a simplified method for writing generators, but what it does, suspending the execution of a function with the ability to resume it, is a much general concept than iteration. It enable a routine to do a small amount of work, and then pass the control to another routine, thus allowing them to run concurrently. This enable data to be passed around in a network of routines. It is a more powerful form of data processing than iterations. In other words, although generators are used for iterations, `yield` doesn't have to be.

The ontology is much simpler in Scheme. Since programming entities in Scheme are not organized hierarchically as classes, you don't have the awkwardness of implementing coroutines with the generator class, which is designed for iterations. In Scheme, coroutines and generators are just normal functions that manipulate continuations in different ways. 
