...
### Stepping into the [IndieWeb](https://indieweb.org)
...
Since I wanted to blog from a Micropub client program as Quill, the first issue was to tell it who I was. I thought I had to create an account on Quill, but the IndieWeb world has its own way of doings things. IndieWeb's protocol for establishing identity, called [IndieAuth](https://indieweb.org/IndieAuth), was confusing to me in the beginning, but it makes lots of sense after I got used to it.

The central philosophy is that my identity on the web should belong to me. Web services should come to my site for all information about me. I shouldn't need to create different accounts for different social networks, and I shouldn't use their methods to authenticate myself. This means that to log in to an IndieWeb-based service, the only information that I need to provide is the address of my website. From my website, the service can extract the location of something called an _authorization endpoint_. The service negotiates with my authorization endpoint to establish my identity.

In theory, this authorization endpoint should be a server program that I write myself to implements _my_ policies. For example, if somebody uses my website to log in to Quill, my authorization endpoint might ask for a password, or even a retinal scan. It can be anything that I consider sufficient to identify myself. But of course, there are easier ways. For example, [IndieAuth.com](https://indieauth.com) is a free authorization endpoint that is popular in the IndieWeb community. The way it works is that I can ask IndieAuth.com to go to one of the web services that I already use to authenticate me. Since I use GitHub a lot, I asked IndieAuth.com to delegate authorization to GitHub. If I can convince GitHub that I am the person who I claim to be, IndieAuth.com will also report to any service that I am that guy, using the IndieAuth protocol. Note that IndieAuth.com is one particular implementation of the IndieAuth protocol. The protocol itself is more general.

[This](https://indieauth.com/setup) is the instructions for setting up for IndieAuth.com. I also found [this tutorial](https://www.amitgawande.com/2018/02/10/204300.html) useful. First, on my site where I link to my GitHub account, I added the `rel=me` attribute to the `<a>` tag to tell GitHub that my site is associated with my account. For the theme that I am using, I had to edit the `layouts/partials/links.html` template, so that in the generated HTML, the link to GitHub looks like this:
```html
...
I forked `gozette`'s repository on `GitHub` so I could have a [personalized copy](https://github.com/hsinhaoyu/gozette). The code did not compile at the first try, because some of the dependent libraries have changed their API. I updated `gozette` slightly to accommodate the changes, and customized it for my site:


1. The file `validation.go` is all about authentication with `IndieAuth`. I assigned the constant `indieAuthMe` to the URL my domain name.
...
### Debugging the server code locally
Before I uploaded `gozette` to AWS Lambda, I made sure that it ran correctly on my own laptop. Since I made several changes to the source code, some debugging was necessary. The good news is that AWS provides a tool called [SAM](https://aws.amazon.com/blogs/aws/new-aws-sam-local-beta-build-and-test-serverless-applications-locally/) for this purpose. We use `sam build` to compile `gozette`, and `sam local start-api` to start the server. As soon as the server is started, a local IP address (such as XXX) is provided by SAM. That is the address of the endpoint. 
...
To make it work, we need to know more about the authentication process. As discussed above, I can log in to a Micropub client (think Quill) using the authorization endpoint IndieAuth.com. The question is: how does the Micropub endpoint (think `gozette`) know that the client is actually acting on my behave? What happens if another person tries to post to my Micropub endpoint? The answer is that after successful authentication, the client passes an "access token" to the Micropub endpoint. Importantly, the client doesn't generate this token itself. Instead, it delegates the job to another endpoint, called the [token endpoint](https://indieweb.org/token-endpoint). When the Micropub endpoint receives the access token, it contacts the token endpoint to verify that the token is valid. The HTML code provided above shows that I opt to use another service (`https://tokens.indieauth.com/token`) to be my token endpoint, although a variety of options are also available. Note that in this setup, the Micropub endpoint doesn't need to know about the authorization endpoint. This is why in `gozette`'s source code, there is no references to the authorization endpoint, but I had to provide the token endpoint.

To communicate to the locally running `gozette` with `curl`, I needed to provide an access token. It's not trivial to find one, because it's not normally exposed to the end-user. Luckily, Sebastiaan Andeweg's [Gimme an Access Token webapp](https://gimme-a-token.5eb.nl) provides this function. After obtaining a token, I could request `gozette` to commit a new post to GitHub with the following command:
